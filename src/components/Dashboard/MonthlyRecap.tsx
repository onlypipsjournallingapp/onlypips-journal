...
const MonthlyRecap: React.FC<MonthlyRecapProps> = ({ trades }) => {
  ...
  const losses = currentMonthTrades.filter(trade => trade.result === 'LOSS').length;
  const breakEven = currentMonthTrades.filter(trade => trade.result === 'BREAK EVEN' || trade.is_break_even).length;
  ...
  const totalPnL = currentMonthTrades.reduce((sum, trade) => sum + Number(trade.profit_loss), 0);
  const avgPnL = totalTrades > 0 ? totalPnL / totalTrades : 0;

  // Get best and worst trades
  const bestTrade = currentMonthTrades.reduce((best, trade) => 
    Number(trade.profit_loss) > Number(best?.profit_loss || -Infinity) ? trade : best, 
    null as Trade | null
  );
  
  const worstTrade = currentMonthTrades.reduce((worst, trade) => 
      ...
      <Card className="glass-card">
        ...
        <CardContent>
          <div className="text-center py-8 text-muted-foreground">
            ...
            <p>No trades recorded this month yet.</p>
          </div>
        </CardContent>
      </Card>
    );
